...
{ // Input and setting initial data
  Config config;
    ...
    input >> config.pos.x >> config.pos.y >> config.zd >> config.target.x >> config.target.y >> config.attackSpeed >> config.accelerationPath >> config.ammo_name;
    input.close();
...
double calculateFireDistance(const Config config, const double flightTime)
{
    ...
    double fireDistance = ft * v
        - ft2 * d * v /(2 * m)
        + ft3 * (6 * dglm - 6 * d2 * (l2 - 1) * v) / (36 * m2)
        + ft4 * (
            -6 * d * dglm * (1 + l2 + l4)
            + 3 * d3 * l2 * (1 + l2) * v
            + 6 * d3 * l4 * (1 + l2) * v
